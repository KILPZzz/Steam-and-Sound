package com.frostybadhalo.create_ss.gui;
...
import com.frostybadhalo.create_ss.registry.SSGuiTextures;
...
public class ADVTrainHUD {

	public static final IGuiOverlay OVERLAY = ADVTrainHUD::renderOverlay;
	...
	public static void tick() {
		...
		if (isSprintKeyPressed && honkPacketCooldown-- <= 0) {
			...
			if (train.lowHonk != null) {
				SSPackets.getChannel().sendToServer(new HonkPacket.Serverbound(train, true));
		...
		if (!isSprintKeyPressed && usedToHonk) {
			SSPackets.getChannel().sendToServer(new HonkPacket.Serverbound(train, false));
		...
		if (hudPacketCooldown-- <= 0) {
			SSPackets.getChannel().sendToServer(new AdvancedHUDPacket.Serverbound(train, editedThrottle));
			hudPacketCooldown = 5;
		}
	}

	private static Carriage getCarriage() {
		if (!(AdvancedControlsHandler.getContraption() instanceof CarriageContraptionEntity cce))
			return null;
	...
	public static void renderOverlay(ForgeGui gui, GuiGraphics graphics, float partialTicks, int width, int height) {
		...
		System.out.println("render custom chamado");
		Minecraft mc = Minecraft.getInstance();
		...
		if (cameraEntity == null)
			return;
		BlockPos localPos = AdvancedControlsHandler.getControlsPos();
		if (localPos == null)
			return;
		...
		SSGuiTextures.TRAIN_HUD_FRAME.render(graphics, -2, 1);
		SSGuiTextures.TRAIN_HUD_SPEED_BG.render(graphics, 0, 0);

		int w = (int) (SSGuiTextures.TRAIN_HUD_SPEED.getWidth() * displayedSpeed.getValue(partialTicks));
		int h = SSGuiTextures.TRAIN_HUD_SPEED.getHeight();

		graphics.blit(SSGuiTextures.TRAIN_HUD_SPEED.location, 0, 0, 0, SSGuiTextures.TRAIN_HUD_SPEED.getStartX(),
				SSGuiTextures.TRAIN_HUD_SPEED.getStartY(), w, h, 256, 256);
		...
		if (promptSize > 1) {
			...
			SSGuiTextures.TRAIN_PROMPT_L.render(graphics, -3, 0);
			SSGuiTextures.TRAIN_PROMPT_R.render(graphics, promptSize, 0);
			graphics.blit(SSGuiTextures.TRAIN_PROMPT.location, 0, 0, 0, SSGuiTextures.TRAIN_PROMPT.getStartX() + (128 - promptSize / 2f),
					SSGuiTextures.TRAIN_PROMPT.getStartY(), promptSize, SSGuiTextures.TRAIN_PROMPT.getHeight(), 256, 256);
		...
		SSGuiTextures.TRAIN_HUD_DIRECTION.render(graphics, 77, -20);

		w = (int) (SSGuiTextures.TRAIN_HUD_THROTTLE.getWidth() * (1 - displayedThrottle.getValue(partialTicks)));
		int invW = SSGuiTextures.TRAIN_HUD_THROTTLE.getWidth() - w;
		graphics.blit(SSGuiTextures.TRAIN_HUD_THROTTLE.location, invW, 0, 0, SSGuiTextures.TRAIN_HUD_THROTTLE.getStartX() + invW,
				SSGuiTextures.TRAIN_HUD_THROTTLE.getStartY(), w, h, 256, 256);
		SSGuiTextures.TRAIN_HUD_THROTTLE_POINTER.render(graphics,
			Math.max(1, SSGuiTextures.TRAIN_HUD_THROTTLE.getWidth() - w) - 3, -2);
